...
fun Flow<DataFirebaseCallModel>.mapToFirebaseCallModelFlowWithUserEntity() = flow{
    this@mapToFirebaseCallModelFlowWithUserEntity.collect{
        val data = it.data
...
fun Flow<DataFirebaseCallModel>.mapToFirebaseCallModelFlow() = flow {
    this@mapToFirebaseCallModelFlow.collect{
        emit(FirebaseCallModel(it.isSuccess, it.data, it.errorMessage))
    }
}
